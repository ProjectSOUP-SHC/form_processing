...
generate_delivery_roster <- function(input, output, session, dels) {
  ...
  print("googlesheets visit history")
  ...
  lookback_date = Sys.Date()-6
  for (l in langs){
    # resp <- read_sheet('https://docs.google.com/spreadsheets/d/1xikHSj8TGHwsYHZUOSdWPWRjSTbOGViPt6_gFtNKrM0/edit#gid=63042318',
                       
    resp <- read_sheet('https://docs.google.com/spreadsheets/d/1m9bbVTLoEgN2Ne8Ns1jgW-f-R7NOw_RANsRDxqX9HQ8/edit#gid=63042318',
                       sheet = l)
    ...
    resp.fin <- resp %>%
      ...
      filter(time>lookback_date|del=="Yes"|del=="Sim"|del=="Wi"|del=="Sí") %>%
  ...
  if(!exists('orders')) {
    stop(paste("no online orders found starting at", lookback_date, "and phone numbers", dels))
  }
  ...
  generated_sheets <- gs4_find(paste("Orders_", Sys.Date(), sep = ''))
  print(generated_sheets)
  # need to extract the id of the first column
...
# 
# generate_delivery_roster(dels = '781-333-0754')
